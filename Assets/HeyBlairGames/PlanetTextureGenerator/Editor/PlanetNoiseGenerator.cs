	...
	public class PlanetNoiseGenerator
	{
		...
		private void cloudNoiseWork( int jobId, int jobCount )
		{
			...
			for( int i = startIndex; i < endIndex; ++i )
			{
				...
				p.x			/= _cloudSpin;
				p.y			/= _cloudSpin;
				float h		= getSurfaceHeight( p, _cloudParams, perlinNoiseCloud );
#if true
				//colour
				...
				cloudData[ index + 3 ]	= ( byte ) Mathf.RoundToInt( a * 255 );
#endif
		...
		private float sample( Vector3 pos, PlanetNoise noiseParams, PlanetNoisePerlin perlinNoise )
		{
			...
			{
				offset = getOctave( pos / noiseParams.smearScale, noiseParams.smearOctaves, perlinNoise );
				offset = Mathf.Pow( offset, noiseParams.smearFalloff );
				offset *= noiseParams.smearIntensity;
			}

			result = getNormalisedOctave( ( pos / noiseParams.scale ) + ( Vector3.one * offset ), noiseParams.octaves, perlinNoise );

			if( noiseParams.ridginess > 0f )
			{
				float ridge	= getNormalisedOctave( ( pos / noiseParams.scale ) + new Vector3( offset, offset, offset + 11f ), noiseParams.octaves, perlinNoise );
		...
		private float getOctave( Vector3 pos, int octaves, PlanetNoisePerlin perlinNoise )
		{
			float result	= 0f;
			float scale		= 1f;
			...
			{
				result	+= perlinNoise.noise( pos * scale ) / scale;
				scale	*= 2f;
			}
		...
		private float getNormalisedOctave( Vector3 pos, int octaves, PlanetNoisePerlin perlinNoise )
		{
			float result	= 0f;
			float scale		= 1f;
