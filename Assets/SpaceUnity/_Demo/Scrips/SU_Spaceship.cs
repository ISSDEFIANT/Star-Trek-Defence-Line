...
public class SU_Spaceship : MonoBehaviour {
	...
	void Update () {
		if (Input.GetKeyDown(KeyCode.R)) {
				RedAlert.GetComponent<AudioSource>().enabled = true;
		}
		if (Input.GetKeyDown(KeyCode.G)) {
			RedAlert.GetComponent<AudioSource>().enabled = false;
		}
		...
		if (Input.GetButtonDown("Fire2")) {
			...
			foreach (Vector3 _wmp in weaponMountPoints) {
				...
				Transform _laserShot = (Transform) Instantiate(laserShotPrefab, StartPos.transform.position, transform.rotation);
	...
	void FixedUpdate () {
		...
		_cacheRigidbody.AddRelativeTorque(new Vector3(0,-Input.GetAxis("Horizontal")*rollRate*_cacheRigidbody.mass));
		...
		_cacheRigidbody.AddRelativeTorque(new Vector3(0,0,Input.GetAxis("Rotate")*QSpeed*_cacheRigidbody.mass));
	}
}
