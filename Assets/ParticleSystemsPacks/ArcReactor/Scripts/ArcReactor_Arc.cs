...
public class ArcReactor_Arc : MonoBehaviour {
	...
	protected bool playBackward = false;
	...
	void Start () 
	{
		...
		for(int n = 0; n < arcs.Length; n++)
		{
			...
			//ColliderParant = new GameObject("Collider");
			//ColliderParant.transform.parent = transform;

	//		Collider = new GameObject("Collider");
	//		Collider.transform.parent = ColliderParant.transform;
	...
	void Update()
	{
		if (shapeTransforms [1] == null) {
			Destroy (gameObject);
		}
		//capsule = gameObject.GetComponentInChildren<CapsuleCollider> ();
		//ColliderParant.transform.position = shapeTransforms[0].position + (shapeTransforms[1].position - shapeTransforms[0].position) / 2;
		//ColliderParant.transform.LookAt(shapeTransforms[0].position);
		//capsule.height = (shapeTransforms[1].position - shapeTransforms[0].position).magnitude;
		//Phase shifting
		for (int n = 0; n < arcs.Length; n++)
		{
		...
		//	foreach (Light child in gameObject.GetComponentsInChildren<Light> ()) {
