...
public class ArcReactor_Launcher : MonoBehaviour {
	...
	public void LaunchRay()
	{
		...
		tmpobj = (GameObject)Instantiate(arcPrefab, transform);
	...
	public void PhaserFire(Transform endtransform)
	{
		if (launchMethod == LaunchMethod.forward_raycast && startBehaviour == RayTransformBehaivour.follow_raycast)
		{
			Debug.LogError("Launch method 'forward_raycast' and start behaviour 'follow_raycast' are incompatible. Change one of the settings.");
		...
		end.transform.position = endtransform.position;
		//if(endtransform.position != new Vector3(0,0,0)){
		//hit.transform.position = endtransform.position;
		//}

		//if (hit.transform.position != new Vector3(0,0,0)) {
		//	end.position = hit.point;
		endObj = endtransform.gameObject;
		//}
		if (endBehaviour == RayTransformBehaivour.stick && endtransform != null)
		{
			end.parent = endtransform;
		}
		else
		{
			end.parent = globalSpaceTransform;
